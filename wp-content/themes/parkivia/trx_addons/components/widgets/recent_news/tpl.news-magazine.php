...
?><article 
	...
	trx_addons_get_template_part('templates/tpl.featured.php',
								...
								apply_filters('trx_addons_filter_args_featured', array(
												...
												'thumb_size' => trx_addons_get_thumb_size($number<=$featured ? 'medium' : 'medium')
	...
	if ( !in_array($post_format, array('link', 'aside', 'status', 'quote')) ) {
		...
		<div class="post_header entry-header">
			...
			the_title( ($number<=$featured ? '<h5' : '<h5').' class="post_title entry-title"><a href="'.esc_url(get_permalink()).'" rel="bookmark">', '</a>'.($number<=$featured ? '</h5>' : '</h5>') );
