...
if (!function_exists('parkivia_hovers_theme_setup3')) {
	add_action( 'after_setup_theme', 'parkivia_hovers_theme_setup3', 3 );
	function parkivia_hovers_theme_setup3() {

		// Add 'Buttons hover' option
		parkivia_storage_set_array_after('options', 'border_radius', array(
			'button_hover' => array(
				"title" => esc_html__("Button's hover", 'parkivia'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme buttons', 'parkivia') ),
				"std" => 'slide_left',
				"options" => array(
					'slide_left'	=> esc_html__('Slide from Left',	'parkivia'),
				),
				"type" => "select"
			),
			'image_hover' => array(
				"title" => esc_html__("Image's hover", 'parkivia'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme images', 'parkivia') ),
				"std" => 'dots',
				"override" => array(
					'mode' => 'page',
				...
				"options" => array(
					'dots'	=> esc_html__('Dots',	'parkivia'),
					'icon'	=> esc_html__('Icon',	'parkivia'),
					'icons'	=> esc_html__('Icons',	'parkivia'),
					'zoom'	=> esc_html__('Zoom',	'parkivia'),
					'fade'	=> esc_html__('Fade',	'parkivia'),
					'slide'	=> esc_html__('Slide',	'parkivia'),
					'pull'	=> esc_html__('Pull',	'parkivia'),
					'border'=> esc_html__('Border',	'parkivia')
				),
				"type" => "select"
			) )
		);
...
if (!function_exists('parkivia_hovers_theme_setup9')) {
	add_action( 'after_setup_theme', 'parkivia_hovers_theme_setup9', 9 );
	function parkivia_hovers_theme_setup9() {
		add_action( 'wp_enqueue_scripts',			'parkivia_hovers_frontend_scripts', 1010 );
		add_filter( 'parkivia_filter_localize_script','parkivia_hovers_localize_script' );
		add_filter( 'parkivia_filter_merge_scripts',	'parkivia_hovers_merge_scripts' );
		add_filter( 'parkivia_filter_merge_styles',	'parkivia_hovers_merge_styles' );
		add_filter( 'parkivia_filter_merge_styles_responsive', 'parkivia_hovers_merge_styles_responsive' );
		add_filter( 'parkivia_filter_get_css', 		'parkivia_hovers_get_css', 10, 2 );
	}
}
	
// Enqueue hover styles and scripts
if ( !function_exists( 'parkivia_hovers_frontend_scripts' ) ) {
	//Handler of the add_action( 'wp_enqueue_scripts', 'parkivia_hovers_frontend_scripts', 1010 );
	function parkivia_hovers_frontend_scripts() {
		if ( parkivia_is_on(parkivia_get_theme_option('debug_mode')) && parkivia_get_file_dir('theme-specific/theme-hovers/theme-hovers.js')!='' )
			wp_enqueue_script( 'parkivia-hovers', parkivia_get_file_url('theme-specific/theme-hovers/theme-hovers.js'), array('jquery'), null, true );
	}
}

// Merge hover effects into single css
if (!function_exists('parkivia_hovers_merge_styles')) {
	//Handler of the add_filter( 'parkivia_filter_merge_styles', 'parkivia_hovers_merge_styles' );
	function parkivia_hovers_merge_styles($list) {
...
if (!function_exists('parkivia_hovers_merge_styles_responsive')) {
	//Handler of the add_filter( 'parkivia_filter_merge_styles_responsive', 'parkivia_hovers_merge_styles_responsive' );
	function parkivia_hovers_merge_styles_responsive($list) {
...
if (!function_exists('parkivia_hovers_localize_script')) {
	//Handler of the add_filter( 'parkivia_filter_localize_script','parkivia_hovers_localize_script' );
	function parkivia_hovers_localize_script($arr) {
		$arr['button_hover'] = parkivia_get_theme_option('button_hover');
		return $arr;
	}
}

// Merge hover effects into single js
if (!function_exists('parkivia_hovers_merge_scripts')) {
	//Handler of the add_filter( 'parkivia_filter_merge_scripts', 'parkivia_hovers_merge_scripts' );
	function parkivia_hovers_merge_scripts($list) {
...
if ( !function_exists('parkivia_hovers_add_icons') ) {
	function parkivia_hovers_add_icons($hover, $args=array()) {
		...
		// Hover style 'Icon'
			...
			?><div class="icons"><a href="<?php echo !empty($post_link) ? esc_url($post_link) : $no_link; ?>" aria-hidden="true" class="icon-001-link"></a></div><?php
...
// Add styles into CSS
if ( !function_exists( 'parkivia_hovers_get_css' ) ) {
	//Handler of the add_filter( 'parkivia_filter_get_css', 'parkivia_hovers_get_css', 10, 2 );
	function parkivia_hovers_get_css($css, $args) {
...
.sc_button_hover_slide_left {	
    background: linear-gradient(to right,	{$colors['text_link3']} 0%, {$colors['text_hover3']} 50%, {$colors['text_link2']} 50%, {$colors['text_hover2']} 100%) no-repeat scroll right bottom / 210% 100% {$colors['alter_link']} !important;
}
...
.sc_button_hover_style_dark.sc_button_hover_slide_left {		
    background: linear-gradient(to right,	{$colors['text_link2']} 0%, {$colors['text_hover2']} 50%, {$colors['text_hover3']} 50%, {$colors['text_link3']} 100%) no-repeat scroll right bottom / 210% 100% {$colors['alter_link']} !important;
 }
