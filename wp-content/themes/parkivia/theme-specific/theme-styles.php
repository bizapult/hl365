...
if (!function_exists('parkivia_customizer_get_css')) {

	function parkivia_customizer_get_css($args=array()) {
		...
		if ($fonts) {
			...
			$fonts = parkivia_customizer_add_theme_fonts($fonts);
...
.parkivia_tabs .parkivia_tabs_titles li a {
...
EOT;
			$rez = apply_filters('parkivia_filter_get_css', $rez, array('fonts' => $fonts));
			$css['fonts'] = $rez['fonts'];
		}
		...
		if ($vars) {
			...
			// Make theme-specific fonts rules
			$vars = parkivia_customizer_add_theme_vars($vars);
			extract($vars);
...
.body_style_boxed_extra .content_container {
	width: $page_extra;
	padding: 0 20px;
}

EOT;
			$rez = apply_filters('parkivia_filter_get_css', $rez, array('vars' => $vars));
			$css['vars'] = $rez['vars'];
		}
		...
		//--------------------------------------
		if ($colors !== false) {
			$schemes = empty($scheme) ? array_keys(parkivia_get_sorted_schemes()) : array($scheme);
	
			if (count($schemes) > 0) {
				$rez = array();
				foreach ($schemes as $s) {
					// Prepare colors
					if (empty($scheme)) $colors = parkivia_get_scheme_colors($s);
...
.content_wrap .wp-block-archives .select_container { 
	border-color: {$colors['input_bd_color']};
}

.content_wrap .wp-block-archives .select_container:hover,
.content_wrap .wp-block-archives .select_container:focus { 
...
.mapplic-element .mapplic-sidebar .mapplic-list-category > a,
.mapplic-element .mapplic-sidebar .mapplic-list-location > a{
...
.footer_wrap .sc_layouts_item_icon.icon-location{
    color: {$colors['text_link']};
}
.footer_wrap .sc_layouts_item_icon.icon-icon_2{
    color: {$colors['text_hover']};
}
...
.sc_layouts_row_type_compact .sc_layouts_item a:not(.sc_button):not(.button):hover .sc_layouts_item_details_line2,
...
.sc_layouts_row_type_compact .sc_layouts_item a:not(.sc_button):not(.button):hover .sc_layouts_item_details_line1 {
	color: {$colors['text_link']};
}
...
.parkivia_tabs .parkivia_tabs_titles li a:hover {
	color: {$colors['inverse_link']};
	background-color: {$colors['text_link']};
}
div.esg-filter-wrapper .esg-filterbutton.selected > span,
.mptt-navigation-tabs li.active a,
.parkivia_tabs .parkivia_tabs_titles li.ui-state-active a {
	color: {$colors['bg_color']};
	background-color: {$colors['text_dark']};
}
...
.sheme_self.sidebar .parkivia_tabs .parkivia_tabs_titles li a {
	color: {$colors['alter_dark']};
	background-color: {$colors['alter_bg_hover']};
}
.sheme_self.sidebar div.esg-filter-wrapper .esg-filterbutton > span:hover,
.sheme_self.sidebar .mptt-navigation-tabs li a:hover,
.sheme_self.sidebar .parkivia_tabs .parkivia_tabs_titles li a:hover {
	color: {$colors['inverse_link']};
	background-color: {$colors['text_link']};
}
...
.sheme_self.sidebar .parkivia_tabs .parkivia_tabs_titles li.ui-state-active a {
...
.post_item_single .post_content .post_meta .post_share .social_item .social_icon {
	color: {$colors['inverse_link']} !important;
	background-color: {$colors['text_link']};
	background-image: linear-gradient(-21deg, rgba(0, 0, 0, 0) 0%, rgba(255, 255, 255, 0.3) 100%);
}
...
.footer_custom_footer-boxed.scheme_self.footer_wrap{
    background-color: {$colors['alter_dark']};
}
...
.scheme_self.footer_wrap ul li a,
...
.scheme_self.footer_wrap ul li a:hover,
...
.footer_wrap.scheme_self .socials_wrap .social_item{
...
EOT;
					...
					$rez = apply_filters('parkivia_filter_get_css', $rez, array('colors' => $colors, 'scheme' => $s));
		...
		return apply_filters( 'parkivia_filter_prepare_css', $css_str, $remove_spaces );
	}
}
?>