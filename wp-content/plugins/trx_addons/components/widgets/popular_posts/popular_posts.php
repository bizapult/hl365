	...
	function widget($args, $instance) {
		...
		for ($i=0; $i<3; $i++) {
			...
			if ($tabs[$i]['orderby'] == 'views') {			// Most popular
				...
				$q_args['orderby'] = 'meta_value_num';
			} else if ($tabs[$i]['orderby'] == 'likes') {	// Most commented
			...
			$q_args = trx_addons_query_add_posts_and_cats($q_args, '', $tabs[$i]['post_type'], $tabs[$i]['cat'], $tabs[$i]['taxonomy']);
				...
				while ($q->have_posts()) { $q->the_post();
					...
					trx_addons_get_template_part('templates/tpl.posts-list.php',
												...
												apply_filters('trx_addons_filter_widget_posts_args',
													array(
														'counters' => $tabs[$i]['orderby'] == 'views' ? 'views' : ($tabs[$i]['orderby'] == 'likes' ? 'likes' : 'comments'),
