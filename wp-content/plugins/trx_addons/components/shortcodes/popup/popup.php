...
if ( !function_exists( 'trx_addons_sc_popup' ) ) {
	function trx_addons_sc_popup($atts, $content=null){	
		...
		if (!empty($atts['content']) && strpos($atts['content'], '[') !== false)
			$atts['content'] = shortcode_unautop($atts['content']);

		// Do shortcodes in the custom content or filter content from the textarea
		$atts['content'] = str_replace('wp-audio-shortcode',
										'wp-audio-shortcode-noinit',
										empty($atts['content']) ? do_shortcode($content) : apply_filters('widget_text_content', $atts['content'])
										);
