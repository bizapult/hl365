...
while ( have_posts() ) { the_post();
	...
	if ($trx_addons_add_contacts) {
		...
		if (!empty($meta['contact_form_shortcode'])) {
			$form = $meta['contact_form_shortcode'];
		}
		if (!empty($form)) {
			?><div class="services_page_contact_form">
				<h3 class="services_contact_form_title"><?php printf(esc_html__('Contact us', 'trx_addons')) ?></h3>
				<?php trx_addons_show_layout( do_shortcode( $form ) ); ?>
			</div><!-- /.sc_form --><?php		}
