...
*/
(function(window, Booki){
	window['Booki'] = Booki || function(){};
})(window, Booki);
(function(){
...
(function(window, $, _){
	Booki.ViewBase = Backbone.View.extend({
		...
		'isValid': function(groupName){
			var isValid = true;
			groupName = groupName || '.booki_parsley_validated';
			$(this.el).find(groupName).each(function(){
				var $elem = $(this),
					result;
...
(function(url){
	Booki.QuantityElement = Booki.ModelBase.extend({
		'url': url
		, 'action': {
			'create': 'booki_insertQuantityElement'
			, 'update': 'booki_updateQuantityElement'
			, 'delete': 'booki_deleteQuantityElement'
			, 'read':   'booki_readQuantityElement'
		}
	});
})(window['ajaxurl']);
(function(url){
	Booki.QuantityElementItem = Booki.ModelBase.extend({
		'url': url
		, 'action': {
			'create': 'booki_insertQuantityElementItem'
			, 'update': 'booki_updateQuantityElementItem'
			, 'delete': 'booki_deleteQuantityElementItem'
			, 'read':   'booki_readQuantityElementItem'
		}
	});
})(window['ajaxurl']);
(function(url){
	Booki.QuantityElementItemDelete = Booki.ModelBase.extend({
		'url': url
		, 'action': {
			'delete': 'booki_deleteQuantityElementItems'
		}
	});
})(window['ajaxurl']);
(function(url){
...
(function(url){
	Booki.QuantityElementsByCalendar = Backbone.Collection.extend({
		'model': Booki.QuantityElement,
		'url': url,
		'action' : {'read': 'booki_readAllQuantityElementsByCalendarId'}
	});
})(window['ajaxurl']);

(function(url){
	Booki.QuantityElementsByCalendarDay = Backbone.Collection.extend({
		'model': Booki.QuantityElement,
		'url': url,
		'action' : {'read': 'booki_readAllQuantityElementsByCalendarDayIdList'}
	});
})(window['ajaxurl']);
...
(function(url, $, moment, accounting){
	Booki.CalendarDayView = Booki.ViewBase.extend({
		'events': {
			...
			, 'click .new-quantity-element.btn': 'newQuantityElement'
			, 'click .update-quantity-element.btn': 'updateQuantityElement'
			, 'click .delete-quantity-element.btn': 'deleteQuantityElement'
			, 'click .update-quantity-element-item.btn': 'updateQuantityElementItem'
			...
			, 'change select[name="quantityElementDisplayMode"]': 'inputChanged'
			, 'change select[name="quantityElementBookingMode"]': 'inputChanged'
			, 'change select[name="quantityElements"]': 'quantityElementsChanged'
			, 'change select[name="seatMode"]': 'inputChanged'
		...
		'initialize': function(config) {
			...
			this.weekdays = [];
			this.quantityElement = this.getDefaultQuantityModel();
			this.quantityElements = new Booki.QuantityElementsByCalendarDay();
			this.quantityValidationGroup = '.booki_parsley_validated_quantity';
		...
		'render': function(attrs){
			...
			seasons.fetch({'success': function(collection, resp){
					...
					context.collection = new Booki.CalendarDays({'calendarId': calendarId, 'seasonName': context.selectedSeason === '0' ? null : context.selectedSeason});
					context.collection.fetch({'success': function(collection, resp){
							...
							context.quantityElement = context.getDefaultQuantityModel();
							context.quantityElements = new Booki.QuantityElementsByCalendarDay({'calendarDayIdList':  context.getCalendarDaysIdList()});
							context.quantityElements.fetch({'success': function(models, resp){
									if(models.length > 0){
										context.quantityElement = models.at(0);
									}
		...
		'renderCalendarDay': function(){
			...
			content = this.template(
			{
				...
				, 'quantityElement': this.quantityElement.toJSON()
				, 'quantityElementIsNew': this.quantityElement.isNew()
				, 'quantityElements': this.quantityElements
			});
			$el.html(content);
			this.$quantitySection = $el.find('#quantitycalendarydaysection');
		...
		'inputChanged': function(e){
			var target = e.currentTarget
				...
				, name = target['name']
				, type = target['type']
				, isQuantityElement = $target.hasClass('quantity-element')
				, value
				, data = {}
				, model = this.model
				, flag1
			...
			switch(name){
				...
				case 'minutes':
					...
					if(value === 0){
						if((name === 'minutes' && model.get('hours') === 0) ||
							(name === 'hours' && model.get('minutes') === 0)){
							value = 1;
						}
					}
					flag1 = true;
				...
				case 'bookingLimit':
					value = parseInt($target.val(), 10);
				break;
				case 'quantityElementName':
					value = $target.val();
					name = 'name';
				break;
				case 'quantityElementQuantity':
					value = parseInt($target.val(), 10);
					name = 'quantity';
				break;
				case 'quantityElementCost':
					value = parseFloat($target.val());
					name = 'cost';
				break;
				case 'quantityElementDisplayMode':
					value = parseInt($target.val(), 10);
					name = 'displayMode';
				break;
				case 'quantityElementBookingMode':
					value = parseInt($target.val(), 10);
					name = 'bookingMode';
				break;
				case 'quantityElementIsRequired': 
					value = $target.is(':checked');
					name = 'isRequired';
				break;
			}
			if(isQuantityElement){
				model = this.quantityElement;
			}
			data[name] = value;
		...
		'quantityElementsChanged': function(e){
			var $target = $(e.currentTarget)
				, quantityIndex = parseInt($target.val(), 10);
			this.quantityElement = this.quantityElements.at(quantityIndex);
			this.renderCalendarDay();
			this.expandQuantitySection();
		},
		'newQuantityElement': function(e){
			this.quantityElement = this.getDefaultQuantityModel();
			this.renderCalendarDay();
			this.expandQuantitySection();
		},
		'updateQuantityElement': function(e){
			var context = this
				, prevQuantity = this.quantityElement.previous('quantity')
				, currentQuantity = this.quantityElement.get('quantity')
				, quantityElementItem;
			if(!this.isValid(this.quantityValidationGroup)){
				return;
			}
			if(this.quantityElement.isNew()){
				this.quantityElements.push(this.quantityElement);
			}
			if(prevQuantity != currentQuantity){
				quantityElementItem = new Booki.QuantityElementItemDelete({'id': this.quantityElement.get('id')});
				quantityElementItem.destroy({'success': 
					function(model, resp){
						context.quantityElement.set({'quantityElementItems': []});
						context.saveQuantityElement();
					}
				});
			}else{
				this.saveQuantityElement();
			}
		},
		'saveQuantityElement': function(){
			var context = this;
			this.quantityElement.set({'calendarId': null, 'calendarDayIdList': this.getCalendarDaysIdList()});
			this.quantityElement.save(this.quantityElement.attributes, { 'success':
				function(model, resp){
					context.renderCalendarDay();
					context.expandQuantitySection();
				} 	
			});
		},
		'deleteQuantityElement': function(e){
			var context = this;
			
			this.quantityElement.destroy({'success': 
				function(model, resp){
					context.quantityElements.remove(context.quantityElement);
					context.quantityElement = context.getDefaultQuantityModel();
					context.renderCalendarDay();
					context.expandQuantitySection();
				}
		...
		'updateQuantityElementItem': function(e){
			var $target = $(e.currentTarget)
				, quantityIndex = parseInt($target.attr('data-booki-index'), 10)
				, item = this.getQuantityElementItem(quantityIndex)
				, quantityElementItem =  new Booki.QuantityElementItem(item)
				, context = this;
			quantityElementItem.save(item.attributes, { 'success':
				function(model, resp){
		...
		'getQuantityElementItem': function(quantityIndex){
			var $textbox = $(this.el).find('input[data-booki-index="' + quantityIndex + '"]')
				, cost = parseFloat($textbox.val())
				, quantityElementId = this.quantityElement.get('id')
				, quantityElementItems = this.quantityElement.get('quantityElementItems')
				, params
				, result
				, flag
				, i;
			for(i = 0; i < quantityElementItems.length; i++){
				result = quantityElementItems[i];
				if(result['quantityIndex'] === quantityIndex){
					result['cost'] = cost;
					result['elementId'] = quantityElementId;
					flag = true;
					break;
				}
			}
			if(!flag){
				params = {
					'elementId': quantityElementId
					, 'quantityIndex': quantityIndex
					, 'cost' : cost
				};
				result = new Booki.QuantityElementItem(params);
				quantityElementItems.push(params);
			}
			return result;
		},
		'expandQuantitySection': function(){
			this.$quantitySection.collapse('show');
		},
		'getCalendarDaysIdList': function(){
			var idList = []
				, i;
			this.collection.each(function(item){
		...
		'getDefaultQuantityModel': function(){
			return new Booki.QuantityElement({
				'calendarId': null
		...
		'getDefaultModel': function(model, day){
			var seasonName =  model ? model.get('seasonName') : ''
				...
				, seatMode = model ? model.get('seatMode') : 1
				, bookingLimit = model ? model.get('bookingLimit') : 0;
...
(function(url, $, moment, accounting){
	Booki.CalendarView = Booki.ViewBase.extend({
		'events': {
			...
			, 'change select[name="quantityElementMode"]': 'inputChanged'
			, 'change select[name="quantityElementDisplayMode"]': 'inputChanged'
			, 'change select[name="quantityElementBookingMode"]': 'inputChanged'
			, 'change select[name="bookingStartLapseMode"]': 'inputChanged'
			, 'change select[name="reminderMode"]': 'inputChanged'
			, 'change select[name="seatMode"]': 'inputChanged'
			, 'change select[name="quantityElements"]': 'quantityElementsChanged'
		...
		'initialize': function(config) {
			...
			this.hourFormat = Booki.HourFormat.H24;
			this.quantityElement = this.getDefaultQuantityModel();
			this.quantityElements = new Booki.QuantityElementsByCalendar();
			this.quantityValidationGroup = '.booki_parsley_validated_quantity';
			if(!this.model){
		...
		'render': function(){
			...
			this.model.fetch({'success': function(model, resp){
					if(typeof(model.get('id')) === 'number' ){
						...
						context.quantityElement = context.getDefaultQuantityModel();
						context.quantityElements = new Booki.QuantityElementsByCalendar({'calendarId':  model.get('id')});
						context.quantityElements.fetch({'success': function(models, resp){
								if(models.length > 0){
									context.quantityElement = models.at(0);
								}
		...
		'renderCalendar': function(refreshCalendarDay){
			var $el
				...
				, modelHasChanged = (!(model.hasChanged('affectedRecords') || model.hasChanged('id'))) && model.hasChanged()
				, isDirty = model.isNew() || modelHasChanged;
			...
			$el.html(content);
			this.$quantitySection = $el.find('#quantitycalendarsection');
			this.createDatePickers();
		...
		'inputChanged': function(e){
			...
			switch(name){
				...
				case 'deposit':
					value = $target.val();
				break;
				case 'quantityElementMode':
					value = parseInt($target.val(), 10);
				break;
				case 'availabilityByQuantityElement':
					value = $target.is(':checked');
				break;
				case 'includePriceInQuantityElement':
					value = $target.is(':checked');
				break;
				case 'bookingStartLapse':
					value = parseInt($target.val(), 10);
				break;
				case 'bookingStartLapseMode':
					value = parseInt($target.val(), 10);
				break;
				case 'reminder':
					value = parseInt($target.val(), 10);
		...
		'updateQuantityElement': function(e){
			...
			this.quantityElement.set({'calendarId': this.model.get('id')});
			if(this.quantityElement.isNew()){
				this.quantityElements.push(this.quantityElement);
			}
			if(prevQuantity != currentQuantity){
		...
		'getDefaultQuantityModel': function(){
			var calendarId = this.model ? this.model.get('id') : null;
			return new Booki.QuantityElement({
	...
	Booki.CascadingListView = Booki.ViewBase.extend({
		...
		'render': function(){
			...
			this.models.fetch({'reset': true, 'success': function(models, resp){
					if(typeof(context.selectedListId) !== 'number'){
			...
			if(this.el){
				collection.fetch({
					success: function(models, resp){
						...
						context.hasFullControl = resp['hasFullControl'];
		...
		'output': function(collection){
			var $el = $(this.el),
				...
				content = this.template({
					...
					, 'hasFullControl': this.hasFullControl
				});
			$el.html(content);
...
(function(url, $, _){
	Booki.ProjectView = Booki.ViewBase.extend({
        ...
        'initialize': function(config) {
			...
			this.hasFullControl = config['hasFullControl'];
			if(!this.model){
				this.model = this.getDefaultModel();
			}
			this.model.bind('change', this.render);
		...
		'getDefaultModel': function(){
			return new Booki.Project({
				...
				, 'attendeeTabLabel': Booki.resx.PROJECT_TAB_ATTENDEE_TAB_LABEL_DEFAULT
