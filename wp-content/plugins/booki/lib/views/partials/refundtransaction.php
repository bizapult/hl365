<?php
	class Booki_RefundTransaction{
		...
		public function __construct(){
			...
			$this->bookedQuantityElement = apply_filters( 'booki_refund_booked_quantityElement', null);
...
<?php if($_Booki_RefundTransaction->orderId): ?>
	<div class="booki-content-box">
		...
		<form class="form-horizontal booki-top-margin" role="form" method="POST" data-parsley-validate action="<?php echo $_SERVER['REQUEST_URI']?>">
			...
			<?php endif; ?>
			<?php if(isset($_Booki_RefundTransaction->bookedQuantityElement)): ?>
			<input type="hidden" name="bookedQuantityElementId" value="<?php echo $_Booki_RefundTransaction->bookedQuantityElement->id ?>" />
			<?php endif; ?>
			...
			<div class="form-group">
				<label class="col-lg-4 control-label" for="refundType" title="<?php echo __('Type of refund being made.', 'booki')?>">
					<?php echo __('Refund Type', 'booki') ?>
				</label>
				<div class="col-lg-8">
					 <p class="form-control-static booki-form-control-static"><pre><?php echo $_Booki_RefundTransaction->refundType ?></pre></p>
				</div>
			</div>
			<div class="form-group">
				<label class="col-lg-4 control-label" for="amount" title="<?php echo __('Amount to be refunded (If RefundType is full, do not set the amount). For Decimal point use a dot [.] and Thousands sep use a comma [,]', 'booki')?>">
					<?php echo __('Refund amount') ?>
				</label>
				<div class="col-lg-8">
					 <p class="form-control-static booki-form-control-static"><pre><?php echo Booki_Helper::toMoney($_Booki_RefundTransaction->amount) ?></pre></p>
				</div>
			</div>
			<div class="form-group">
				<label class="col-lg-4 control-label" for="currency" title="<?php echo __('Currency must be the same currency type used to make the 
							payment that is being refunded.', 'booki')?>">
						<?php echo __('Currency', 'booki') ?>
				</label>
				<div class="col-lg-8">
					 <p class="form-control-static booki-form-control-static"><pre><?php echo $_Booki_RefundTransaction->currency ?></pre></p>
				</div>
			</div>
			<div class="form-group clearfix">
				<label class="col-lg-4 control-label" for="refundSource" title="<?php echo __('By default this will use any available funding source', 'booki')?>">
