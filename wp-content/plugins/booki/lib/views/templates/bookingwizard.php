...
<?php endif; ?>
<?php if(isset($_Booki_BookingWizardTmpl->data->seatRequired) && $_Booki_BookingWizardTmpl->data->seatRequired):?>
<div class="bg-danger booki-bg-box">
	<?php echo $_Booki_BookingWizardTmpl->resx->BOOKING_WIZARD_SEAT_REQUIRED_LOC ?>
</div>
<?php endif; ?>
<?php if(isset($_Booki_BookingWizardTmpl->data->quantityElementRequired) && $_Booki_BookingWizardTmpl->data->quantityElementRequired):?>
<div class="bg-danger booki-bg-box">
	<?php echo $_Booki_BookingWizardTmpl->resx->BOOKING_WIZARD_QUANTITY_ELEMENT_REQUIRED_LOC ?>
</div>
<?php endif; ?>
<div id="bookiwizard_validation_<?php echo $_Booki_BookingWizardTmpl->projectId ?>" class="bg-danger booki-bg-box hide">
...
<div id="bookiwizard<?php echo $_Booki_BookingWizardTmpl->projectId ?>" class="booki col-lg-12 booki-no-padding">
	...
	<div class="booki <?php echo $_Booki_BookingWizardTmpl->project->bookingWizardMode === Booki_BookingWizardMode::TABS ? 'tab-content' : '' ?> form-horizontal">
		<div
			...
			<div class="col-lg-12">
				<?php if(!$_Booki_BookingFormTmpl->data->bookingsExhausted): ?>
					...
					<?php Booki_ThemeHelper::includeTemplate('quantityelement.php') ?>
					<div class="booki-vertical-gap"></div>
		...
		<?php endif; ?>
		<?php if($_Booki_BookingWizardTmpl->attendeeList): ?>
		<div 
			<?php if($_Booki_BookingWizardTmpl->project->bookingWizardMode === Booki_BookingWizardMode::TABS):?>
			class="tab-pane" 
			<?php endif;?>
			id="attendeetab<?php echo $_Booki_BookingWizardTmpl->projectId ?>">
				<div class="col-lg-12">
					<div class="form-group">
						<?php if($_Booki_BookingWizardTmpl->project->bookingWizardMode === Booki_BookingWizardMode::LINEAR):?>
						<div class="col-lg-8 col-lg-offset-4">
						<?php endif; ?>
							<?php $_Booki_BookingWizardTmpl->attendeeList->display();?>
						<?php if($_Booki_BookingWizardTmpl->project->bookingWizardMode === Booki_BookingWizardMode::LINEAR):?>
						</div>
						<?php endif; ?>
					</div>
						...
						<?php if (is_user_logged_in() || !$_Booki_BookingWizardTmpl->globalSettings->membershipRequired): ?> 
							<?php if($_Booki_BookingWizardTmpl->globalSettings->enableBookingWithAndWithoutPayment || !$_Booki_BookingWizardTmpl->globalSettings->enablePayments):?>
								<button type="submit" 
										...
										<?php echo ($_Booki_BookingWizardTmpl->globalSettings->enablePayments && $_Booki_BookingWizardTmpl->globalSettings->enableBookingWithAndWithoutPayment) ? $_Booki_BookingWizardTmpl->resx->BOOK_NOW_PAY_LATER_LOC : $_Booki_BookingWizardTmpl->resx->BOOK_NOW_LOC ?>
								</button>
							<?php endif; ?>
...
<script type="text/javascript">
	(function() {
		...
		function onload(e){
			new Booki.Booking(<?php echo $_Booki_BookingFormTmpl->data->toJson() ?>);
			new Booki.BookingWizard({
				"elem": "#bookiwizard" + <?php echo $_Booki_BookingWizardTmpl->projectId ?>
				, "projectId": <?php echo $_Booki_BookingWizardTmpl->projectId ?>
				, "bookingButton": ".booki-booking-button"
				, "nextButton": ".booki-booking-next"
				, "backButton": ".booki-booking-back"
				, "step1": "a[data-step=\"0\"]"
				, "tabs": ".booki<?php echo $_Booki_BookingWizardTmpl->projectId ?>.nav.nav-tabs a"
