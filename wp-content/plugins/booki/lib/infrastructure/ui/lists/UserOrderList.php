...
class Booki_UserOrderList extends Booki_List {
  ...
  function __construct($userId = null){
		...
		if($this->hasFullControl && $userId !== null){
			$this->userId = $userId;
		}else{
	...
	function column_tasks($item){
		$buttonGroups = array();
		$selectUrl = esc_url(add_query_arg(array('orderid'=>$item['id'], 'timezone'=>false)));
		...
		if($enablePayments){
			...
			array_push($buttonGroups, sprintf(
				...
				, $paymentUrl
				, __('Pay', 'booki')
				, Booki_Helper::formatCurrencySymbol(Booki_Helper::toMoney($totalAmount), true)
	...
	*/
    function bind() {
        ...
        $result = $orderRepository->readAll($current_page, $per_page, $this->orderBy, $this->order, null, null, $this->userId, null, null, true);
