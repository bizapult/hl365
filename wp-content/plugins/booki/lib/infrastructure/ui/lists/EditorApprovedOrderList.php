...
class Booki_EditorApprovedOrderList extends Booki_List {
	...
	function column_token($item){
		...
		if ($item['status'] == Booki_PaymentStatus::UNPAID && $canEdit){
			array_push($buttonGroups, sprintf(
				'<li>
	...
	*/
    function bind() {
        ...
        $result = $orderRepository->readAll($this->currentPage, $this->perPage, $this->orderBy, $this->order, $fromDate, $toDate, $userId, $status, null, $this->hasFullControl);
