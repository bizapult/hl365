  ...
  function __construct( ){
	...
	function column_bookingsCount($item){
		...
		array_push($buttonGroups, sprintf(
			...
			, esc_url(add_query_arg('userid', $item['id'], remove_query_arg('orderid')))
			, __('Select', 'booki')
			, $item['bookingsCount']
		));
		...
		array_push($buttonGroups, sprintf(
			...
			,  esc_url(add_query_arg(array('selecteduserid'=>$item['id'], 'command'=>'delete'), remove_query_arg('orderid')))
			, __('Delete', 'booki')
		));
