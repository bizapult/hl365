...
class Booki_EventsLogList extends Booki_List {
  ...
  function __construct(){
	...
	function column_id($item){
		$result = sprintf('<div class="eventslog-col"><span>%s</span></div>'
			, $item['id'] 
		);
		return $result;
	}
	...
	function column_entryDate($item){
		return sprintf('<div class="eventslog-col">%s</div>', $item['entryDate']->format(get_option('date_format')));
	}
	...
	function column_data($item){
		...
		var_dump($item['data']);
		$result = ob_get_clean();
		return sprintf('<div class="eventslog-data">%s</div>', $result);
	}
