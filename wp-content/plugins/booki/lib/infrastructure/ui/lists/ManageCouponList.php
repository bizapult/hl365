...
class Booki_ManageCouponList extends Booki_List {
	...
	function column_tasks($item){
		...
		array_push($buttonGroups, sprintf(
			...
			, esc_url(add_query_arg('couponid', $item['id']))
			, __('Select', 'booki')
		));
		...
		if(!$item['emailedTo'] && !$expired){
			...
			array_push($buttonGroups, sprintf(
				...
				, esc_url(add_query_arg(array('couponid'=>$item['id'], 'command'=>'email')))
    ...
    function column_orderMinimum($item){
		return sprintf('<span title="%s">%s</span>'
			...
			, Booki_Helper::formatCurrencySymbol(Booki_Helper::toMoney($item['orderMinimum']), true)
		);
    }
	...
	*/
    function bind() {
        ...
        $result = $couponRepository->readAll($this->currentPage, $this->perPage, $this->orderBy, $this->order, $userId);
