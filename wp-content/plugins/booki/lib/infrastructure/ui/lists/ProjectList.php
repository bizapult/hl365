  ...
  function __construct($listArgs ){
	...
	function get_full_layout($item){
		...
		$url .= 
			...
			'&dispalyallresultsbydefault=' . $this->listArgs['dispalyAllResultsByDefault'];
		...
		if($bookingEnded){
			...
			if(!$this->isWidget){
				$result .= sprintf('<p class="booki-project-list-date" title="%s"><span class="label label-primary">%s - %s</span></p>'
					...
					, $this->localizedWPDateFormat($startDate)
					, $this->localizedWPDateFormat($endDate)
				);
	...
	*/
    function bind() {
		...
		$totalItems = 0;
		$this->totalPages = 0;
		if($this->dispalyAllResultsByDefault || ($fromDate && $toDate)){
