...
class Booki_SettingsGlobalController extends Booki_BaseController{
	private $repo;
	private $id;
	public function __construct($createCallback = null, $updateCallback = null, $deleteCallback = null, $regenerateCustomPagesCallback = null){
		if (!(array_key_exists('controller', $_POST) 
			&& $_POST['controller'] == 'booki_globalsettings')){
				return;
		}
		...
		if (array_key_exists('booki_regenerate_custompages', $_POST)){
			$this->regenerateCustomPages($regenerateCustomPagesCallback);
		}
	}
	public function regenerateCustomPages($callback){
		Booki_Admin::deleteAllCustomPages();
		$result = Booki_Admin::registerCustomPages();
	...
	protected function process($id = -1) {
		...
		$calendarFirstDay = (int)$this->getPostValue('calendarFirstDay');
		...
		$orderExpiryMode = (int)$this->getPostValue('orderExpiryMode');
		...
		$enableBookMoreButton = $this->getBoolPostValue('enableBookMoreButton');
		...
		$refParsleyJS = $this->getBoolPostValue('refParsleyJS');
		$refMomentJS = $this->getBoolPostValue('refMomentJS');
		...
		$enablePayPalBilling = $this->getBoolPostValue('enablePayPalBilling');
		$autoApproveAfterBillingSettlement = $this->getBoolPostValue('autoApproveAfterBillingSettlement');
		$registerUserAfterPayment = $this->getBoolPostValue('registerUserAfterPayment');
		$enableMobileInitialScale = $this->getBoolPostValue('enableMobileInitialScale');
		...
		$globalSettings->orderExpiryMode = $orderExpiryMode;
		...
		$globalSettings->enableBookMoreButton = $enableBookMoreButton;
		...
		$globalSettings->refParsleyJS = $refParsleyJS;
		$globalSettings->refMomentJS = $refMomentJS;
		...
		$globalSettings->enablePayPalBilling = $enablePayPalBilling;
		$globalSettings->autoApproveAfterBillingSettlement = $autoApproveAfterBillingSettlement;
		$globalSettings->registerUserAfterPayment = $registerUserAfterPayment;
		$globalSettings->enableMobileInitialScale = $enableMobileInitialScale;
