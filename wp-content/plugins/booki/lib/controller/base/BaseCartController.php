...
class Booki_BaseCartController extends Booki_BaseController{
	...
	public function addToCart($callback = null){
		...
		$this->validateCart();
		$result = Booki_BookingFormParser::populateBookingFromPostData($this->projectId, $this->cart->getBookings());
		$this->cart->rebuildSession($result['bookings']);
	...
	public function removeDate($callback = null){
		...
		$temp = explode(',', $_POST['booki_remove_date']);
		foreach($temp as $t){
			$value = explode(':', $t);
			$id = intval($value[0]);
	...
	public function checkOut($callback = null, $cartCallback = null){
		...
		$hasDiscount = ($discount > 0 && ($bookingMinimumDiscount == 0 || $this->cart->count() >= $bookingMinimumDiscount));
		$bookings = $this->cart->getBookings();
		if($hasDiscount){
		...
		if($globalSettings->autoInvoiceNotification && ($order->userIsRegistered || !$globalSettings->membershipRequired)){
	...
	*/
	...
	protected function validateDays()
	{
		...
		foreach($bookings as $booking){
			...
			if(!$bookedDays){
				$bookedDays = $bookedDaysRepository->readByDays(array($booking->date), $projectId);
			}
