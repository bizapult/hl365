...
class Booki_PaypalSetting extends Booki_EntityBase
{
	...
	public function __construct()
	{
		...
		if($numArgs > 0)
		{
			...
			$this->enableGuestCheckout = func_get_arg(16);
			if($numArgs === 18)
			{
				$this->id = func_get_arg(17);
			}
			if(!isset($this->itemCategory))
			{
				$this->itemCategory = 'Digital';
			}
			if(!isset($this->enableGuestCheckout)){
				$this->enableGuestCheckout = false;
			}
		}
	}
}
