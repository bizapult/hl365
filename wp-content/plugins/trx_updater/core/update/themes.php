...
class Themes extends Base {
	...
	private function get_theme_info() {
		...
		if ( ! is_array( $data ) || ! empty( $_GET['force-check'] ) ) {
			...
			$response = trx_updater_fgc( $this->update_url . '?action=info_theme&theme_slug=' . $this->theme_slug );
	...
	public function update_theme() {
		...
		if ( ! empty( $nonce ) && ! empty( $theme ) && $theme == $this->theme_slug && wp_verify_nonce( $nonce, "update-theme_{$theme}" ) && current_user_can( 'update_themes' ) ) {
			...
			$theme_url = sprintf( $this->update_url . '?action=install_theme&src=%1$s&key=%2$s&theme_slug=%3$s&theme_name=%4$s&rnd=%5$s',
